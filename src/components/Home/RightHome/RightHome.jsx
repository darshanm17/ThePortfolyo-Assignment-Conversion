import React from "react";
import Projectimg from "./Projectimg";

export default function RightHome() {
  return (
    <>
      <div class="right-side">
        <div class="container">
          <div class="copyright-home copyright-home-hidden"></div>
        </div>

        <div class="home-img-container">
          <div class="slide-img">
            <div class="hero-fullscreen">
              <div class="hero-fullscreen-FIX">
                <div class="hero-bg">
                  <div class="swiper-container-wrapper">
                    <div class="swiper-container">
                      <div class="swiper-wrapper">
                        <div class="swiper-slide">
                          <div
                            class="swiper-slide-inner"
                            data-swiper-parallax="50%"
                          >
                            <div class="swiper-slide-inner-bg bg-img-1"></div>

                            <div class="overlay overlay-dark-60"></div>

                            <div class="swiper-slide-inner-txt">
                              <p>Beauty / Fashion</p>

                              <div class="divider-s"></div>

                              <h2>Simplicity is complex</h2>
                            </div>
                          </div>
                        </div>

                        <div class="swiper-slide">
                          <div
                            class="swiper-slide-inner"
                            data-swiper-parallax="50%"
                          >
                            <div class="swiper-slide-inner-bg bg-img-2"></div>

                            <div class="overlay overlay-dark-60"></div>

                            <div class="swiper-slide-inner-txt">
                              <p>People / Life</p>

                              <div class="divider-s"></div>

                              <h2>Design is a process</h2>
                            </div>
                          </div>
                        </div>

                        <div class="swiper-slide">
                          <div
                            class="swiper-slide-inner"
                            data-swiper-parallax="50%"
                          >
                            <div class="swiper-slide-inner-bg bg-img-3"></div>

                            <div class="overlay overlay-dark-60"></div>

                            <div class="swiper-slide-inner-txt">
                              <p>Royalty / Stock</p>

                              <div class="divider-s"></div>

                              <h2>Aesthetic is a decision</h2>
                            </div>
                          </div>
                        </div>

                        <div class="swiper-slide">
                          <div
                            class="swiper-slide-inner"
                            data-swiper-parallax="50%"
                          >
                            <div class="swiper-slide-inner-bg bg-img-4"></div>

                            <div class="overlay overlay-dark-60"></div>

                            <div class="swiper-slide-inner-txt">
                              <p>Technology / Stack</p>

                              <div class="divider-s"></div>

                              <h2>Style is everything</h2>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div class="swiper-bottom">
                    <div class="swiper-slide-pagination"></div>

                    <div class="hero-slider-bg-controls-wrapper">
                      <div class="hero-slider-bg-controls">
                        <div class="swiper-slide-controls swiper-slide-controls-dark slide-prev">
                          <div class="ion-ios-arrow-left"></div>
                        </div>
                        <div class="swiper-slide-controls slide-next">
                          <div class="ion-ios-arrow-right"></div>
                        </div>
                      </div>

                      <div class="swiper-slide-controls-play-pause-wrapper swiper-slide-controls-play-pause slider-on-off">
                        <div class="slider-on-off-switch">
                          <i class="ion-ios-play"></i>
                        </div>

                        <div class="slider-progress-bar">
                          <span>
                            <svg class="circle-svg" height="50" width="50">
                              <circle
                                class="circle"
                                cx="25"
                                cy="25"
                                fill="none"
                                r="24"
                                stroke="#ffffff"
                                stroke-width="2"
                              ></circle>
                            </svg>
                          </span>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="about-img-container mobile overlay overlay-dark-60">
          <div class="slide-img">
            <img
              alt="BG Img"
              src="https://portfolio-image-store.s3.ap-south-1.amazonaws.com/portfolio3/1710357726628-c4dr18.png"
            />
          </div>
        </div>

        <div class="works-img-container mobile overlay overlay-dark-60">
          <div class="slide-img">
            <img
              alt="BG Img"
              src="https://portfolio-image-store.s3.ap-south-1.amazonaws.com/1706286058630-v3ax0a"
            />
          </div>
        </div>

        <div class="services-img-container mobile overlay overlay-dark-60">
          <div class="slide-img">
            <img
              alt="BG Img"
              src="https://d6xcmfyh68wv8.cloudfront.net/learn-content/uploads/2022/08/shutterstock_experience_letter-1024x683.jpg"
            />
          </div>
        </div>

        <div class="news-img-container mobile overlay overlay-dark-60">
          <div class="slide-img">
            <img
              alt="BG Img"
              src="https://portfolio-image-store.s3.ap-south-1.amazonaws.com/1706285368529-zdr5fl"
            />
          </div>
        </div>

        <div class="contact-img-container mobile overlay overlay-dark-60">
          <div class="slide-img">
            <img
              alt="BG Img"
              src="https://portfolio-image-store.s3.ap-south-1.amazonaws.com/1706283290608-n4hq7k"
            />
          </div>
        </div>
      </div>
    </>
  );
}
